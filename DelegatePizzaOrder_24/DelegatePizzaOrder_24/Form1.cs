...
namespace DelegatePizzaOrder_24
{
    public partial class Form1 : DevExpress.XtraEditors.XtraForm
    {
        ...
        // 함수 명 뒤에 <일반화 변수명 - 형식 매개 변수> 을
        // 사용 할 경우 일반화 함수를 지정 할수 있음 (인자 값 Type에 신경 쓰지 않는 형태)
        public delegate T delFunc<T>(T i);
        // 문자 종류는 상관 없음 일반화 함수를 Delegate에도 적용할 수 있다는 점
        // (확장개념 일반화 delegate를 delegate의 특징이라고 보기는 어렵지 않을지)
        public delegate A delTestc<A>(A i);
        ...
        private void frmLoadind(Dictionary<string, int> dPizzaOrder)
        {
            if (fPizza != null)
            {
                fPizza.Dispose();
